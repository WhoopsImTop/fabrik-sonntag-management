<template>
  ...
  <div v-else class="max-w-7xl mx-auto space-y-6 pb-24 font-sans text-slate-900">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
      <!-- Kunde Auswahl -->
      <div class="lg:col-span-1 border border-slate-200 bg-white text-slate-950">
        <div class="flex flex-col space-y-1.5 p-6 pb-4">
          <div class="flex items-center justify-between">
            <h3 class="font-semibold leading-none tracking-tight">Kunde</h3>
            <span v-if="form.user_id"
              class="inline-flex items-center  border border-emerald-200 px-2.5 py-0.5 text-xs font-semibold bg-emerald-100 text-emerald-800 transition-colors">Ausgewählt</span>
            <span v-else
              class="inline-flex items-center  border border-slate-200 px-2.5 py-0.5 text-xs font-semibold bg-slate-100 text-slate-500 transition-colors">Neu</span>
          </div>
          ...
          <div class="space-y-2">
            ...
            <div class="relative group">
              ...
              <input type="text" v-model="userSearchQuery" @focus="showUserDropdown = true" @blur="closeUserDropdown"
                class="flex h-9 w-full  border border-slate-200 bg-transparent px-3 py-1 pl-9 text-sm  transition-colors placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:cursor-not-allowed disabled:opacity-50"
            ...
            <div v-if="showUserDropdown && filteredUsers.length > 0"
              class="absolute z-50 mt-1 w-[calc(100%-3rem)] md:w-[330px]  border border-slate-200 bg-white text-slate-950  outline-none">
              <div class="max-h-60 overflow-y-auto p-1">
                <div v-for="user in filteredUsers" :key="user.id" @click="selectUser(user)"
                  class="relative flex cursor-pointer select-none items-center  px-2 py-1.5 text-sm outline-none hover:bg-slate-100 hover:text-slate-900">
                  <div class="flex items-center gap-2">
                    <div
                      class="flex h-6 w-6 shrink-0 items-center justify-center bg-slate-100 text-xs font-medium text-slate-900">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-2">
                ...
                <input v-model="customerForm.first_name"
                  class="flex h-9 w-full  border border-slate-200 bg-transparent px-3 py-1 text-sm  transition-colors placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950"
                  placeholder="Max" />
              </div>
              <div class="space-y-2">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <!-- Einstellungen -->
        <div class="border border-slate-200 bg-white text-slate-950">
          ...
          <div class="p-6 space-y-6">
            ...
            <div class="space-y-3">
              ...
              <div class="grid grid-cols-2 gap-4">
                <label class="relative flex cursor-pointer  border p-4 hover:bg-slate-50 transition-colors"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
              ...
              <div class="space-y-2">
                ...
                <select v-model="form.status"
                  class="flex h-9 w-full items-center justify-between  border border-slate-200 bg-transparent px-3 py-2 text-sm  focus:outline-none focus:ring-1 focus:ring-slate-950 disabled:cursor-not-allowed disabled:opacity-50">
        ...
        <div class="border border-slate-200 bg-white text-slate-950">
          <div class="flex items-center justify-between p-6 pb-4 border-b border-slate-100">
            ...
            <button @click="addItem"
              class="inline-flex h-8 items-center justify-center rounded-none border border-slate-200 bg-white px-3 text-xs font-medium  transition-colors hover:bg-slate-100 hover:text-slate-900 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 gap-1.5">
                  ...
                  <tr class="transition-colors hover:bg-slate-50/50 group">
                    <td class="p-4 align-middle relative">
                      <div class="relative">
                        <input v-model="item.description" @input="handleInput(index)" @focus="focusRow(index)"
                          @blur="blurRow(index)"
                          class="flex h-9 w-full border border-neutral-200  px-3 py-1 text-sm transition-colors placeholder:text-slate-400 focus-visible:outline-none focus:border-slate-300 focus:bg-white"
                          placeholder="Leistung eingeben..." />

                        <div v-if="focusedRowIndex === index && suggestions.length > 0"
                          class="absolute z-50 left-0 top-full mt-1 w-[300px]  border border-slate-200 bg-white  outline-none"
                          @mousedown.prevent>
                          <div class="p-1 max-h-60 overflow-y-auto">
                            <div v-for="sugg in suggestions" :key="sugg.id" @click="applySuggestion(index, sugg)"
                              class="relative flex cursor-pointer select-none items-center justify-between  px-2 py-1.5 text-sm outline-none hover:bg-slate-100 hover:text-slate-900">
                              <div class="flex flex-col">
                    ...
                    <td class="p-4 align-middle text-right">
                      <input type="number" v-model="item.quantity" min="1"
                        class="flex h-9 w-full border border-slate-200 text-right  px-3 py-1 text-sm focus-visible:outline-none focus:border-slate-300 focus:bg-white" />
                    </td>

                    <td class="p-4 align-middle text-left">
                      <input type="text" v-model="item.unit"
                        class="flex h-9 w-full border border-slate-200  px-3 py-1 text-sm text-slate-500 focus-visible:outline-none focus:border-slate-300 focus:bg-white"
          ...
          <div class="bg-slate-50 border-t border-slate-200 p-4 pb-6">
