  ...
  <div v-else class="max-w-7xl mx-auto space-y-6 pb-24">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex items-center gap-3">
        <button @click="router.back()" class="p-2 hover:bg-neutral-100 rounded-full text-neutral-500 transition-colors">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="bg-white border border-neutral-200 rounded-xl shadow-sm overflow-hidden">
          <div class="p-5 border-b border-neutral-100 bg-neutral-50/50 flex justify-between items-center">
            <h3 class="font-semibold text-neutral-900 flex items-center gap-2">
              <svg class="w-4 h-4 text-neutral-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
              Kunde
            </h3>
          ...
          <div class="p-5 space-y-5">
            <div class="relative">
              <label class="block text-xs font-medium text-neutral-500 mb-1.5 uppercase">Kunden suchen</label>
              ...
              <div v-if="showUserDropdown && filteredUsers.length > 0" class="absolute z-20 left-0 top-full mt-1 w-full bg-white rounded-lg shadow-xl border border-neutral-100 max-h-60 overflow-y-auto">
                <div 
                ...
                >
                  <div class="w-8 h-8 rounded-full bg-neutral-100 flex items-center justify-center text-xs font-bold text-neutral-600 shrink-0">
                    {{ user.username.charAt(0).toUpperCase() }}
                  </div>
                  <div class="truncate">
                    <div class="text-sm font-medium text-neutral-900 truncate">{{ user.details?.company || user.username }}</div>
            ...
            <div :class="{'opacity-75 pointer-events-none': form.user_id}" class="space-y-4 transition-opacity">
              <div class="grid grid-cols-2 gap-3">
                 <div class="space-y-1">
                   <label class="text-xs font-medium text-neutral-500">Vorname</label>
                   <input v-model="customerForm.first_name" class="w-full px-3 py-2 bg-white border border-neutral-200 rounded-lg text-sm focus:ring-1 focus:ring-neutral-900 focus:border-neutral-900 placeholder:text-neutral-300" placeholder="Max">
                 </div>
                 <div class="space-y-1">
                   <label class="text-xs font-medium text-neutral-500">Nachname</label>
                   <input v-model="customerForm.last_name" class="w-full px-3 py-2 bg-white border border-neutral-200 rounded-lg text-sm focus:ring-1 focus:ring-neutral-900 focus:border-neutral-900 placeholder:text-neutral-300" placeholder="Mustermann">
                 </div>
              </div>

              <div class="space-y-1">
                 <label class="text-xs font-medium text-neutral-500">Firma</label>
                 <input v-model="customerForm.company" class="w-full px-3 py-2 bg-white border border-neutral-200 rounded-lg text-sm focus:ring-1 focus:ring-neutral-900 focus:border-neutral-900 placeholder:text-neutral-300" placeholder="Muster GmbH">
              </div>

              <div class="space-y-1">
                 <label class="text-xs font-medium text-neutral-500">E-Mail</label>
                 <input v-model="customerForm.email" type="email" class="w-full px-3 py-2 bg-white border border-neutral-200 rounded-lg text-sm focus:ring-1 focus:ring-neutral-900 focus:border-neutral-900 placeholder:text-neutral-300" placeholder="max@beispiel.de">
              </div>

              <div class="space-y-1">
                 <label class="text-xs font-medium text-neutral-500">Straße</label>
                 <input v-model="customerForm.street" class="w-full px-3 py-2 bg-white border border-neutral-200 rounded-lg text-sm focus:ring-1 focus:ring-neutral-900 focus:border-neutral-900 placeholder:text-neutral-300" placeholder="Hauptstr. 1">
              </div>

              <div class="grid grid-cols-3 gap-3">
                 <div class="space-y-1 col-span-1">
                   <label class="text-xs font-medium text-neutral-500">PLZ</label>
                   <input v-model="customerForm.zip_code" class="w-full px-3 py-2 bg-white border border-neutral-200 rounded-lg text-sm focus:ring-1 focus:ring-neutral-900 focus:border-neutral-900 placeholder:text-neutral-300" placeholder="12345">
                 </div>
                 <div class="space-y-1 col-span-2">
                   <label class="text-xs font-medium text-neutral-500">Stadt</label>
                   <input v-model="customerForm.city" class="w-full px-3 py-2 bg-white border border-neutral-200 rounded-lg text-sm focus:ring-1 focus:ring-neutral-900 focus:border-neutral-900 placeholder:text-neutral-300" placeholder="Berlin">
                 </div>
              </div>
            </div>
          </div>
      ...
      <div class="lg:col-span-2 space-y-6">
          ...
          <div class="space-y-4">
             ...
             <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
               <div>
                  ...
                  <select v-model="form.interval" class="block w-full pl-3 pr-10 py-2 text-sm border-neutral-200 rounded-lg focus:ring-neutral-900 focus:border-neutral-900 bg-white">
               ...
               <div>
                  ...
                  <input type="date" v-model="form.next_billing_date" class="block w-full text-sm border-neutral-200 rounded-lg focus:ring-neutral-900 focus:border-neutral-900" />
               </div>

               <div>
                  <label class="block text-xs font-medium text-neutral-500 uppercase mb-1">Status</label>
                  <select v-model="form.status" class="block w-full pl-3 pr-10 py-2 text-sm border-neutral-200 rounded-lg focus:ring-neutral-900 focus:border-neutral-900">
        ...
        <div class="bg-white border border-neutral-200 rounded-xl shadow-sm overflow-hidden min-h-[400px] flex flex-col">
           ...
           <table class="w-full text-sm text-left">
            <thead class="bg-neutral-50 text-neutral-500 font-medium border-b border-neutral-100">
              <tr>
            ...
            <tbody class="divide-y divide-neutral-100">
              ...
              >
                <td class="px-6 py-3 relative">
                  <div class="relative">
                    ...
                    >
                        ...
                        >
                          <div>
                            <div class="text-sm font-medium text-neutral-900">{{ sugg.label }}</div>
                            <div class="text-[10px] uppercase tracking-wider text-neutral-400 font-semibold">{{ sugg.type }}</div>
                          </div>
                          <div class="text-xs font-bold text-neutral-700 bg-neutral-100 px-2 py-1 rounded group-hover/item:bg-white">
                ...
                <td class="px-4 py-3 text-right">
                  <input type="number" v-model="item.quantity" min="1" class="w-full text-right bg-transparent border-0 border-b border-transparent focus:border-neutral-400 focus:ring-0 p-0 text-sm" placeholder="1">
                </td>

                <td class="px-4 py-3 text-right">
                   <input type="number" v-model="item.amount" step="0.01" class="w-full text-right bg-transparent border-0 border-b border-transparent focus:border-neutral-400 focus:ring-0 p-0 text-sm" placeholder="0.00">
                </td>
                ...
                <td class="px-2 text-center">
                   <button @click="removeItem(index)" class="text-neutral-300 hover:text-red-500 transition-colors p-1" title="Löschen">
            ...
            <tfoot class="bg-neutral-50 border-t border-neutral-200">
              <tr>
                <td colspan="3" class="px-6 py-3 text-right text-neutral-600">Netto</td>
                <td class="px-6 py-3 text-right text-neutral-900">€{{ formatMoney(totals.net) }}</td>
                <td></td>
              </tr>
              <tr>
                <td colspan="3" class="px-6 py-1 text-right text-neutral-600">USt (19%)</td>
                <td class="px-6 py-1 text-right text-neutral-900">€{{ formatMoney(totals.tax) }}</td>
                <td></td>
              </tr>
              <tr>
                <td colspan="3" class="px-6 py-4 text-right font-bold text-lg text-neutral-900">Gesamtbetrag</td>
                <td class="px-6 py-4 text-right font-bold text-lg text-neutral-900">€{{ formatMoney(totals.gross) }}</td>
                <td></td>
              </tr>
            </tfoot>
          </table>
...
const allProducts = computed(() => {
  const list = []
  services.value.forEach(s => list.push({ id: `svc-${s.id}`, label: s.name, price: s.price_per_unit, type: 'Service' }))
  resources.value.forEach(r => {
    const plans = allPricing.value.filter(p => p.resource_id === r.id)
    if (plans.length > 0) plans.forEach(plan => list.push({ id: `res-${r.id}-p-${plan.id}`, label: `${r.name} - ${plan.name}`, price: plan.price, type: 'Raum' }))
  })
  return list
})
...
const suggestions = computed(() => {
  ...
  const currentInput = form.value.items[focusedRowIndex.value].description.toLowerCase()
  if (!currentInput) return allProducts.value.slice(0, 5)
  return allProducts.value.filter(p => p.label.toLowerCase().includes(currentInput)).slice(0, 8)
})
...
// Table Handling
const focusRow = (index: number) => { focusedRowIndex.value = index }
const blurRow = (index: number) => { setTimeout(() => { if (focusedRowIndex.value === index) focusedRowIndex.value = null }, 200) }
const handleInput = (index: number) => { /* Reactivity trigger */ }
...
const addItem = () => form.value.items.push({ description: '', quantity: 1, amount: 0, vat_rate: 0.19 })
const removeItem = (index: number) => {
  if (form.value.items.length > 1) form.value.items.splice(index, 1)
  else form.value.items[0] = { description: '', quantity: 1, amount: 0, vat_rate: 0.19 }
}
