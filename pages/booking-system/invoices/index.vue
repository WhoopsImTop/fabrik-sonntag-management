<template>
  <div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-3xl font-bold tracking-tight text-neutral-900">Rechnungen</h1>
      ...
      <div class="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm">
        <div class="flex items-center justify-between">
          <div>
            ...
            <h3 class="text-2xl font-bold text-neutral-900 mt-1">€{{ formatMoney(stats.totalRevenue) }}</h3>
          </div>
      ...
      <div class="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm">
        <div class="flex items-center justify-between">
          <div>
            ...
            <h3 class="text-2xl font-bold text-neutral-900 mt-1">€{{ formatMoney(stats.pendingAmount) }}</h3>
          </div>
      ...
      <div class="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm">
        <div class="flex items-center justify-between">
          <div>
            ...
            <h3 class="text-2xl font-bold text-neutral-900 mt-1">€{{ formatMoney(stats.overdueAmount) }}</h3>
          </div>
    ...
    <div class="bg-white border border-neutral-200 rounded-xl shadow-sm overflow-hidden">
      ...
      <div v-else-if="filteredInvoices.length === 0" class="p-12 text-center">
        <div class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-neutral-100 mb-4">
          <svg class="w-6 h-6 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
        </div>
        <h3 class="text-lg font-medium text-neutral-900">Keine Rechnungen gefunden</h3>
        <p class="text-neutral-500 mt-1">Versuche es mit anderen Suchbegriffen oder erstelle eine neue Rechnung.</p>
      </div>

      <table v-else class="min-w-full divide-y divide-neutral-200">
        <thead class="bg-neutral-50/50">
          <tr>
            <th class="px-6 py-3 text-left text-xs font-semibold text-neutral-500 uppercase tracking-wider">Nr. / Datum</th>
            <th class="px-6 py-3 text-left text-xs font-semibold text-neutral-500 uppercase tracking-wider">Kunde</th>
            <th class="px-6 py-3 text-left text-xs font-semibold text-neutral-500 uppercase tracking-wider">Betrag</th>
            <th class="px-6 py-3 text-left text-xs font-semibold text-neutral-500 uppercase tracking-wider">Status</th>
            <th class="px-6 py-3 text-right text-xs font-semibold text-neutral-500 uppercase tracking-wider">Aktion</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-neutral-200 bg-white">
          ...
          >
            <td class="px-6 py-4">
              <div class="text-sm font-medium text-neutral-900">{{ invoice.invoice_number }}</div>
              <div class="text-xs text-neutral-500">{{ formatDate(invoice.created_at) }}</div>
            </td>
            <td class="px-6 py-4">
              <div class="text-sm text-neutral-900">{{ invoice.User?.username || invoice.customer_name || 'Unbekannt' }}</div>
              <div class="text-xs text-neutral-500">{{ invoice.User?.email || invoice.customer_email }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm font-medium text-neutral-900">€{{ formatMoney(invoice.total_amount || invoice.total) }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <span :class="['px-2.5 py-0.5 rounded-full text-xs font-medium border', getStatusClass(invoice.status)]">
...
const calculateStats = (data: any[]) => {
  stats.value.totalRevenue = data.filter(i => i.status === 'PAID').reduce((sum, i) => sum + Number(i.total_amount||0), 0)
  stats.value.pendingAmount = data.filter(i => ['PENDING', 'SENT', 'DRAFT'].includes(i.status)).reduce((sum, i) => sum + Number(i.total_amount||0), 0)
  stats.value.overdueAmount = data.filter(i => i.status === 'OVERDUE').reduce((sum, i) => sum + Number(i.total_amount||0), 0)
}
