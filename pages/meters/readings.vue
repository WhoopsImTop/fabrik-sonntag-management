...
const deleteReading = async (id: number) => {
  const confirmed = window.confirm('Möchten Sie diesen Zählerstand wirklich löschen?');
  if (confirmed) {
...
const deleteAllReadings = async () => {
  const confirmed = window.confirm('ACHTUNG: Möchten Sie wirklich ALLE Zählerstände unwiderruflich löschen?');
  if (confirmed) {
    ...
    <div
      ...
      <UTable :data="readings" :columns="columns" :loading="loading" class="h-full">
        ...
        <template #processing_status-cell="{ row }">
          <UBadge
            :color="row.original.processing_status === 'parsed' ? 'primary' : row.original.processing_status === 'error' ? 'error' : 'neutral'">
        ...
        <template #recordsData-cell="{ row }">
          <div class="flex flex-col gap-1 text-xs">
            ...
            <div v-else v-for="(rec, i) in row.original.records" :key="i" class="flex gap-2">
              ...
              <span>{{ rec.wert_formatiert || rec.wert_roh }} {{ rec.einheit || '' }}</span>
            </div>
          </div>
        </template>
