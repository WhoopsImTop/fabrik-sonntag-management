<template>
  <div class="flex items-center justify-center md:grid md:grid-cols-2 md:bg-gray-100 h-screen">
    ...
    <div class="flex flex-col items-center justify-center bg-gray-100">
      ...
      <h2 class="text-2xl font-bold text-center">Passwort zuruecksetzen</h2>
      <p class="text-sm text-neutral-600 text-center max-w-xs">
        Gib deine E-Mail-Adresse ein. Du erhaelst einen Link zum Zuruecksetzen.
      </p>
...
const requestReset = async () => {
  ...
  try {
    const response = await fetch(
      `${import.meta.env.VITE_INTERNAL_API_URL}/auth/request-password-reset`,
      {
        method: 'POST',
    ...
    const data = await response.json().catch(() => ({}))
    message.value = data?.message || 'Wenn ein Konto existiert, wurde eine E-Mail gesendet.'
  } catch (e) {
