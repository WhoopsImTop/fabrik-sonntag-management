  ...
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" v-if="modelValue">
    <div class="bg-white rounded-lg p-4 w-full max-w-lg">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold">{{ editingRenter ? 'Mieter bearbeiten' : 'Neuer Mieter' }}</h3>
        <button
          type="button"
          @click="closeModal"
        ...
        <div class="pt-4 border-t">
          <div class="flex items-center gap-2 mb-4">
            ...
            <label class="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" v-model="form.hasArea" class="sr-only peer">
              <div class="w-11 h-6 bg-neutral-200 peer-checked:bg-yellow-400 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all"></div>
            </label>
          </div>
          ...
          <div v-if="form.hasArea" class="space-y-4">
            ...
            <div class="relative border rounded-lg" style="height: 300px;">
              ...
              <div class="absolute top-2 right-2 bg-white rounded-lg shadow-sm p-2 flex flex-col gap-2">
                <button
                  @click="startDrawing"
            ...
            <div class="border rounded-lg p-4">
              ...
              <p v-else class="text-sm text-neutral-500 italic">Noch keine Etagen vorhanden</p>
            </div>

            <!-- Floor Form Modal -->
            <div v-if="showFloorForm" class="fixed inset-0 bg-black/50 flex items-center justify-center z-[60]">
...
onMounted(() => {
  ...
  // Load floors when editing a renter with an area
  if (props.editingRenter?.area?.id) {
    loadFloors();
  }
});
...
async function loadFloors() {
  if (!props.editingRenter?.area?.id) return;
...
watch(() => form.value.hasArea, async (hasArea) => {
