<template>
  <div class="h-full flex flex-col bg-white border-l border-slate-200 shadow-2xl font-sans" v-if="booking">
    ...
    <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-start bg-white shrink-0">
      <div>
        <h3 class="text-lg font-bold text-slate-900 tracking-tight">Buchungsdetails</h3>
        <p class="text-xs text-slate-500 font-mono mt-0.5">ID: #{{ booking.id }}</p>
      </div>
      <button 
        @click="$emit('close')" 
    ...
    <div class="flex-1 overflow-y-auto p-6 space-y-8">
      ...
      <div 
        ...
        :class="statusStyles[booking.status]?.bg || 'bg-slate-50 border-slate-200'"
      >
        <div 
          class="p-2 rounded-full shrink-0"
          :class="statusStyles[booking.status]?.iconBg || 'bg-slate-200 text-slate-500'"
        >
      ...
      <div class="space-y-4">
        <div>
           ...
           <h4 class="text-xl font-bold text-slate-900 leading-tight">{{ booking.Resource?.name || 'Unbekannt' }}</h4>
        </div>
        ...
        <div class="flex items-start gap-3 p-4 bg-slate-50 border border-slate-100 rounded-xl">
          ...
          <div>
            <p class="text-sm font-semibold text-slate-900">
              ...
              <span v-if="formatDate(booking.start_at) !== formatDate(booking.end_at)" class="text-slate-400"> bis </span>
              <span v-if="formatDate(booking.start_at) !== formatDate(booking.end_at)">{{ formatDate(booking.end_at) }}</span>
            </p>
            <p class="text-sm text-slate-500 mt-0.5">
              {{ formatTime(booking.start_at) }} – {{ formatTime(booking.end_at) }} Uhr
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-3">
        <h5 class="text-xs font-bold uppercase tracking-widest text-slate-900 border-b border-slate-100 pb-2">Kunde</h5>
        
        <div class="flex items-center gap-4">
          <div class="h-10 w-10 shrink-0 rounded-full bg-slate-900 text-white flex items-center justify-center font-bold text-sm shadow-sm ring-2 ring-slate-50">
          ...
          <div class="min-w-0">
            ...
            <a v-if="booking.User?.email" :href="'mailto:' + booking.User.email" class="text-xs text-blue-600 hover:underline truncate block">
              {{ booking.User.email }}
            </a>
          </div>
        </div>
      ...
      <div class="space-y-3">
        <div class="flex items-center justify-between border-b border-slate-100 pb-2">
           <h5 class="text-xs font-bold uppercase tracking-widest text-slate-900">Rechnung</h5>
           <span v-if="booking.Invoice" class="text-[10px] font-mono text-slate-400">{{ booking.Invoice.invoice_number }}</span>
        </div>

        <div v-if="booking.Invoice" class="group relative rounded-xl border border-slate-200 overflow-hidden hover:border-slate-300 transition-colors">
          <div class="bg-slate-50/50 px-4 py-3 flex justify-between items-center">
             <div class="flex flex-col">
               <span class="text-xs font-medium" :class="booking.Invoice.status === 'PAID' ? 'text-emerald-600' : 'text-amber-600'">
                 {{ booking.Invoice.status === 'DRAFT' ? 'Entwurf' : (booking.Invoice.status === 'PAID' ? 'Bezahlt' : 'Offen') }}
               </span>
             </div>
             <div class="text-right">
               <span class="block text-sm font-bold text-slate-900">{{ formatCurrency(booking.Invoice.total_amount) }}</span>
             </div>
          </div>
          <button 
          ...
          >
             <svg v-if="isDownloading" class="animate-spin w-3 h-3" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
        ...
        <div v-else class="p-3 rounded-lg border border-dashed border-slate-200 text-center">
...
const emit = defineEmits(['close', 'edit', 'cancel', 'update-status'])

const isDownloading = ref(false)

// --- ICONS & STYLES ---
const Icons = {
  Check: h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': 2 }, [h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M5 13l4 4L19 7' })]),
  Clock: h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': 2 }, [h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' })]),
  X: h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': 2 }, [h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M6 18L18 6M6 6l12 12' })])
}

const statusStyles: any = {
  'CONFIRMED': { 
    label: 'Bestätigt', desc: 'Die Buchung ist aktiv.', 
    bg: 'bg-emerald-50 border-emerald-100', text: 'text-emerald-700', iconBg: 'bg-emerald-100', icon: Icons.Check 
  },
  'PENDING': { 
    label: 'Ausstehend', desc: 'Wartet auf Bestätigung.', 
    bg: 'bg-amber-50 border-amber-100', text: 'text-amber-700', iconBg: 'bg-amber-100', icon: Icons.Clock 
  },
  'CANCELLED': { 
    label: 'Storniert', desc: 'Diese Buchung wurde abgesagt.', 
    bg: 'bg-slate-50 border-slate-200', text: 'text-slate-500', iconBg: 'bg-slate-200', icon: Icons.X 
  }
}
...
// --- HELPERS ---
const formatDate = (iso: string) => iso ? new Date(iso).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '-'
const formatTime = (iso: string) => iso ? new Date(iso).toLocaleTimeString('de-DE', { hour: '2-digit', minute:'2-digit' }) : '--:--'
const formatCurrency = (val: string | number) => new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format(Number(val || 0))
